...
export default async function PosReturnPage(props: Params) {
  ...
  if (!customer) {
    redirect(`/${params.countryCode}/account/?redirect=/pos/locations/${params.handle}/store/return`)
  }
  ...
  const t = await getTranslations("PosPage.Return")
  ...
  return (
    <>
      <FirestoreDeviceProvider location={location} mode={mode} type={type}>
        <Start items={steps} title={t("title")} location={location} debug={debug} region={region} mode={mode} type={type} />
